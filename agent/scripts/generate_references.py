...
def cli_version() -> str:
    ...
    Not `ddpsrun.__version__`: that reports whatever is installed in the
...
# Stamping it makes the check `ddpsrun --version` and the fix `pip install -U
# ddpsrun`. A release therefore has to regenerate these pages, which is what
...
BANNER = (
    ...
    f"<!-- Generated from ddpsrun {cli_version()} in this repository. If a flag "
    "here is rejected, compare `ddpsrun --version` and run "
    "`pip install -U ddpsrun`. -->\n"
)
